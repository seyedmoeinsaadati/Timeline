using System;
using UnityEditor;
using UnityEngine;

namespace Moein.TimeSystem
{
    public class TimelineAnimator : Timeline
    {
        private int headIndex;

        protected override void Init()
        {
            pointer = headIndex = -1;
            maxTimelineCaptureCount = Mathf.RoundToInt(recordingTime / captureInterval);
            base.Init();
        }

        public override void Progress(float timescale)
        {
            CalculateLerping(timescale);
            if (rewindableAnimator != null)
                rewindableAnimator.SetSpeed(timescale);

            if (pointer < headIndex)
            {
                if (saveMemory)
                {
                    // forwarding
                    ApplyComponents();

                    if (rewindableAnimator != null && animatorPointer + 1 <= animatorHeadIndex)
                    {
                        if (timelineTime > AnimatorCurrentNextCapturingTime)
                        {
                            animatorPointer++;
                            animatorComponent.ApplySnapshot(animatorComponent.Tape[animatorPointer]);
                        }
                    }

                    return;
                }

                headIndex = pointer;
            }

            Capture();
        }

        public override void Rewind(float timescale)
        {
            CalculateLerping(timescale);
            if (pointer == headIndex)
            {
                transformTimeline.ApplySnapshot(transformTimeline.LerpSnapshot(transformTimeline.HeadSnapshot,
                    transformTimeline.CurrentSnapshot, t));
                return;
            }

            ApplyComponents();

            if (rewindableAnimator != null)
            {
                rewindableAnimator.SetSpeed(timescale);

                if (animatorPointer >= 0)
                {
                    if (AnimatorCurrentCapturingTime > timelineTime)
                    {
                        animatorComponent.ApplySnapshot(animatorComponent.Tape[animatorPointer]);
                        animatorPointer--;
                    }
                }
            }
        }

        protected override void CalculateLerping(float timeScale)
        {
            timelineTime += Time.fixedDeltaTime * timeScale;
            timelineTime = Mathf.Clamp(timelineTime, 0, recordingTime);
            pointer = (int) (timelineTime / captureInterval);
            t = (timelineTime - pointer * captureInterval) / captureInterval;
        }

        public override void Capture()
        {
            if (timelineTime >= captureInterval * (headIndex + 1))
            {
                headIndex++;

                if (headIndex >= maxTimelineCaptureCount)
                {
                    transformTimeline.Tape.RemoveAt(0);
                    headIndex = maxTimelineCaptureCount - 1;
                    timelineTime -= captureInterval;
                }

                CaptureComponents();
            }
        }

        #region TimelineComponents

        // animator
        protected int animatorHeadIndex;
        protected int animatorPointer;
        protected float AnimatorCurrentCapturingTime => animatorComponent.Tape[animatorPointer].time;
        protected float AnimatorCurrentNextCapturingTime => animatorComponent.Tape[animatorPointer + 1].time;
        [SerializeField] protected RewindableAnimator rewindableAnimator = null;
        protected AnimatorComponent animatorComponent = null;

        protected override void InitComponents()
        {
            transformTimeline = new TransformComponent(transform, maxTimelineCaptureCount);

            if (rewindableAnimator != null)
            {
                Debug.Log($"{typeof(TimelineBase)}: Animator Initialized.");

                animatorHeadIndex = animatorPointer = -1;
                animatorComponent = new AnimatorComponent(rewindableAnimator);
                rewindableAnimator.OnChangeAnimator(CaptureAnimator);
            }
        }

        protected override void CaptureComponents()
        {
            transformTimeline.CaptureSnapshot(headIndex);
        }

        protected override void ApplyComponents()
        {
            transformTimeline.ApplySnapshot(transformTimeline.LerpSnapshot(pointer, pointer + 1, t));
        }

        protected virtual void CaptureAnimator(AnimatorSnapshot snapshot)
        {
            snapshot.time = timelineTime;
            animatorHeadIndex++;
            animatorPointer = animatorHeadIndex;
            animatorComponent?.CaptureSnapshot(snapshot);

            Debug.Log($"{typeof(TimelineBase)}: Animator Captured. Time: {timelineTime}");
        }

        #endregion

        private void Reset()
        {
            rewindableAnimator = GetComponentInChildren<RewindableAnimator>();
        }
    }

#if UNITY_EDITOR

    [CustomEditor(typeof(TimelineAnimator)), CanEditMultipleObjects]
    public class TimelineAnimatorEditor : Editor
    {
        public override void OnInspectorGUI()
        {
            DrawDefaultInspector();

            serializedObject.Update();
            if (Application.isPlaying)
            {
                GUILayout.Space(5);
                var timelineTime = serializedObject.FindProperty("timelineTime");
                GUILayout.Label($"Timeline Time: {timelineTime.floatValue}");
            }

            // read-only mode
            // if (Application.isPlaying == false)
            // {
            //     DrawDefaultInspector();
            // }
            // else
            // {
            //     serializedObject.Update();
            //
            //     var saveMemory = serializedObject.FindProperty("saveMemory");
            //     var captureInterval = serializedObject.FindProperty("captureInterval");
            //     var recordingTime = serializedObject.FindProperty("recordingTime");
            //     var timelineTime = serializedObject.FindProperty("timelineTime");
            //
            //     GUILayout.Label($"Save Memory: {saveMemory}");
            //     GUILayout.Label($"Capturing Interval: {captureInterval.floatValue}");
            //     GUILayout.Label($"Recording Time: {recordingTime.floatValue}");
            //     GUILayout.Space(5);
            //     GUILayout.Label($"Timeline Time: {timelineTime.floatValue}");
            // }
        }
    }

#endif
}